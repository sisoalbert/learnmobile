...
function StepContent({
  ...
  switch (stepId) {
    ...
    case 'practice-reminders':
      return <CenteredStep style={styles.topAligned}><SectionTitle>{title}</SectionTitle>{body ? <SupportingText>{body}</SupportingText> : null}<NotificationCard><ReminderChoice value="disabled" label="Not now" selected={state.reminderPreference === 'disabled'} onPress={state.setReminderPreference} /><ReminderChoice value="enabled" label="Allow" selected={state.reminderPreference === 'enabled'} onPress={state.setReminderPreference} /></NotificationCard><SupportingText>After account setup, your device will ask for notification permission. You can change access later in device settings.</SupportingText></CenteredStep>;
